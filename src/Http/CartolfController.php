...
$seo = \Sitedigitalweb\Pagina\Tenant\Cms_seo::where('id','=',1)->get(); 
$meta = \Sitedigitalweb\Pagina\Tenant\Page::where('slug','=','1')->get();

$cart = session()->get('cart');
$url = \Sitedigitalweb\Carrito\Tenant\Cms_configuracion::where('id', '=', 1)->get();
$iva = $this->iva();
$total = $this->total();
$subtotal = $this->subtotal();
$descuento = $this->descuento();
...
return view('templates.cart', compact('cart', 'total', 'subtotal', 'iva', 'descuento', 'url', 'seo', 'departamento'));
}
...
{
    $producto = $this->getProductoBySlug($id);
    ...
    return redirect('/cart/show')->with('success', 'Producto agregado al carrito');
}
...
private function getProductoBySlug($slug)
{
    ...
    return $modelClass::where('slug', $slug)->first(); // Usar first() en lugar de get()[0]
}
...
private function subtotal()
{
    ...
    if($cart != null && !empty($cart)) {
        foreach($cart as $item) {
            ...
            if(property_exists($item, 'preciodescfin') && $item->preciodescfin > 0) {
                $precio = $item->preciodescfin;
            } elseif(property_exists($item, 'precioinivafin') && $item->precioinivafin > 0) {
                $precio = $item->precioinivafin;
            } elseif(property_exists($item, 'precioivafin') && $item->precioivafin > 0) {
                $precio = $item->precioivafin;
            } elseif(property_exists($item, 'precio') && $item->precio > 0) {
                $precio = $item->precio;
            } elseif(property_exists($item, 'preciodesc') && $item->preciodesc > 0) {
                $precio = $item->preciodesc;
            }
            
            $cantidad = property_exists($item, 'quantity') ? $item->quantity : 1;
...
private function descuento()
{
    ...
    if($cart != null && !empty($cart)) {
        foreach($cart as $item) {
            // Obtener precio base
            $precio = 0;
            if(property_exists($item, 'precio') && $item->precio > 0) {
                $precio = $item->precio;
            } elseif(property_exists($item, 'preciodescfin') && $item->preciodescfin > 0) {
                $precio = $item->preciodescfin;
            } elseif(property_exists($item, 'precioivafin') && $item->precioivafin > 0) {
                $precio = $item->precioivafin;
            } elseif(property_exists($item, 'precioinivafin') && $item->precioinivafin > 0) {
            ...
            if(property_exists($item, 'descuento') && $item->descuento > 0) {
                $porcentajeDescuento = $item->descuento;
            }
            
            // Calcular descuento
            $cantidad = property_exists($item, 'quantity') ? $item->quantity : 1;
            $descuento += ($precio * $porcentajeDescuento / 100) * $cantidad;
        }
    }
...
private function iva()
{
    ...
    if($cart != null) {
        foreach($cart as $item) {
            // Verificar qué propiedad de IVA existe
            if(isset($item->precioiva)) {
                $iva += $item->precioiva * $item->quantity;
            } elseif(isset($item->precioiniva)) {
                $iva += $item->precioiniva * $item->quantity;
            } elseif(isset($item->iva)) {
...
$precio = DB::table('cms_municipios')->where('id', '=', session()->get('miSesionTextouno'))->get();
$nombremunicipio = 0;
...
return view('Templates.order', compact('cart', 'total', 'subtotal','configuracion','price','suma', 'iva', 'descuento', 'costoenvio', 'precioenvio', 'preciomunicipio', 'nombremunicipio','departamento'));
...
$cat_id = Input::get('cat_id');
if(!$this->tenantName){
        $subcategories = Cms_municipio::where('departamento_id', '=', $cat_id)->get();
    }else{
    $subcategories = \Sitedigitalweb\Carrito\Tenant\Cms_municipio::where('departamento_id', '=', $cat_id)->get();
    }
        return Response::json($subcategories);
}
