...
class UserController extends Controller{
  ...
  $hostname = app(\Hyn\Tenancy\Environment::class)->hostname();
    ...
    // Método privado para obtener el modelo de usuario según el tenant
private function getUserModel()
{
    return empty($this->tenantName) 
        ? User::class 
        : \Sitedigitalweb\Carrito\Tenant\User::class;
}
...
{
    $userModel = $this->getUserModel();
    $usuario = $userModel::with('orders')->find($id); // Eager loading para evitar N+1
    
    if (!$usuario) {
        abort(404, 'Usuario no encontrado');
    }
    
    $ordenes = $usuario->orders;
